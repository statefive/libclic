 ...
 * how this is processed is explained in {@link Clc}. Once the call to
 * {@link Clc#process(java.io.InputStream, java.lang.String, java.lang.String[])}
 ...
 * the call to process the file defined in {@link Clc}, let's take a look at an
 * example implementation of {@code MyAppListener} within the
 * {@code option(String, Object)} method - members are {@code public} so that
 * once processing of arguments is complete, external classes can obtain the
...
public interface OptionListener {
    ...
    /**
     * Update with the specified option and it's value (if it has one) for
     * root-level options; if the update is from a command (like
     * {@code git diff} for example,
     * {@link #option(java.lang.String, java.lang.String, java.lang.Object)}
     * will be called. Note that the option will be a short option if only short
     * options are defined, a long option if only long options are defined, or
     * both if both short and long options are defined. In the latter case
     * listeners will receive two updates and can decide which form to cater
     * for; in general long options aid readability in source files compared to
     * single characters.
     *
